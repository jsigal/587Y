...
import timeit
import andy_gaus as ag
import sailajam as sm
import nicholasb as nb
import kamils as ks
import connerh as ch
...
def test_a_func(func):
    ...
    for rn,val in test_rn.items():
        ...
        if results[rn] is False:
            print(f'function {func.__name__} failed on {rn} returned {ret} instead of {val}')
    return results
...
# test_a_func(ag.romanToIntCleaner)
...
func_dict = {"previous": ri.romanToInt_prev, "rep_chain" : ri.romanToInt_replace_chain, "replace" : ri.romanToInt_replace,
             ...
             "andygaus" : ag.romanToIntCleaner, "sailajam" : sm.roman_to_int, "nicholasb" : nb.romanToInteger,
             "kamils" : ks.roman2int, "connorh" : ch.romanToInt}
time_dict = dict()
loop = 10000
